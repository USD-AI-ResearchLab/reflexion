...
from util import summarize_react_trial, log_react_trial, save_agents
from retrieval_agents_no_mmr import ReactReflectAgent, ReactAgent, ReflexionStrategy
...
# ── Save log ────────────────────────────────────────────────────────────────
base_path = os.path.join(root, 'ReAct', strategy.value + '_no_mmr')
file_path = os.path.join(base_path, f'{len(agents)}_questions_{trial}_trials.txt')
os.makedirs(os.path.dirname(file_path), exist_ok=True)
with open(file_path, 'w') as f:
    f.write(log)
