...
#     ReflexionStrategy,
# )
# from retrieval_agents import TrajectoryStore   # ← RAR trajectory store

# hotpot   = joblib.load('../data/hotpot-qa-distractor-sample.joblib').reset_index(drop=True)
# strategy = ReflexionStrategy.STAR

# # Both stores shared across all agents and all trials
# trajectory_store = TrajectoryStore()    # ← RAR: stores full trajectories + reflections
# knowledge_store  = StepKnowledgeStore() # ← STAR: stores step-level rules
...
#     print(f'Finished Trial {trial} | Correct: {len(correct)} | '
#           f'Incorrect: {len(incorrect)} | Halted: {len(halted)}')
#     print(f'  TrajectoryStore: {len(trajectory_store.records)} trajectories')
#     print(f'  KnowledgeStore:  {len(knowledge_store.knowledge)} rules')
...
#           f'{len(agents)}_questions_{trial}_trials.txt'), 'w') as f:
#     f.write(log)
...
#     positives   = [sk for sk in knowledge_store.knowledge if sk.positive]
#     corrections = [sk for sk in knowledge_store.knowledge if not sk.positive]
#     f.write(f"Total rules: {len(knowledge_store.knowledge)}\n")
#     f.write(f"Confirmed: {len(positives)} | Corrections: {len(corrections)}\n\n")
#     for i, sk in enumerate(knowledge_store.knowledge, 1):
#         icon = "✓" if sk.positive else "✗"
#         f.write(f"{i}. {icon} Intent: {sk.action_intent}\n")
#         f.write(f"   Rule:   {sk.rule}\n\n")
...
# plt.plot(trial_numbers, success_rates, marker='o', linewidth=2,
#          markersize=6, color='steelblue')
# plt.xlabel('Number of Trials'); plt.ylabel('Success Rate')
# plt.title('STAR — Success Rate vs Trials (HotPotQA)')
# plt.xticks(trial_numbers); plt.ylim(0, 1)
# plt.grid(True, linestyle='--', alpha=0.6); plt.tight_layout()
# plt.savefig(os.path.join(base_path,
#     f'{len(agents)}_questions_success_rate.png'), dpi=150)
# plt.close()
...
# plt.plot(trial_numbers, incorrect_rates, marker='^', linewidth=2,
#          markersize=6, color='darkorange', label='Incorrect Rate')
# plt.xlabel('Number of Trials'); plt.ylabel('Rate')
# plt.title('STAR — Halt vs Incorrect (HotPotQA)')
# plt.xticks(trial_numbers); plt.ylim(0, 1); plt.legend()
# plt.grid(True, linestyle='--', alpha=0.6); plt.tight_layout()
# plt.savefig(os.path.join(base_path,
#     f'{len(agents)}_questions_halt_incorrect.png'), dpi=150)
# plt.close()

# plt.figure(figsize=(8, 5))
# plt.plot(trial_numbers, avg_steps, marker='D', linewidth=2,
#          markersize=6, color='mediumseagreen')
# plt.xlabel('Number of Trials'); plt.ylabel('Average Steps')
# plt.title('STAR — Avg Steps per Trial (HotPotQA)')
# plt.xticks(trial_numbers)
# plt.ylim(0, max(avg_steps) * 1.2 if avg_steps else 10)
# plt.grid(True, linestyle='--', alpha=0.6); plt.tight_layout()
# plt.savefig(os.path.join(base_path,
#     f'{len(agents)}_questions_avg_steps.png'), dpi=150)
# plt.close()
...
# Knowledge store shared across all agents and all trials
knowledge_store = StepKnowledgeStore()
...
for i in range(n):
    ...
    halt_rate    = len(halted)    / total if total > 0 else 0
    inc_rate     = len(incorrect) / total if total > 0 else 0
    steps        = float(np.mean([a.step_n for a in active_agents])) \
...
# Knowledge snapshot
with open(os.path.join(base_path, 'knowledge_snapshot.txt'), 'w') as f:
    positives   = [sk for sk in knowledge_store.knowledge if sk.positive]
    corrections = [sk for sk in knowledge_store.knowledge if not sk.positive]
    f.write(f"Total rules: {len(knowledge_store.knowledge)}\n")
    f.write(f"Confirmed: {len(positives)} | Corrections: {len(corrections)}\n\n")
    for i, sk in enumerate(knowledge_store.knowledge, 1):
        ...
        f.write(f"{i}. [{icon}] Key: {sk.action_intent}\n")
        f.write(f"   Rule: {sk.rule}\n\n")
...
with open(csv_path, 'w') as f:
    ...
    for t, s, fa, h, st in zip(trial_numbers, success_rates,
                                incorrect_rates, halt_rates, avg_steps):
        f.write(f'{t},{s:.4f},{fa:.4f},{h:.4f},{st:.4f}\n')
print(f'Done. Results saved to {base_path}')