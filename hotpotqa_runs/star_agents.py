...
class STARReactAgent:
    ...
    def _build_agent_prompt(self, knowledge_str: str = '') -> str:
        ...
        parts.append(f"Question: {self.question}")
        parts.append(f"Scratchpad:\n{self.scratchpad}")
        # ── Show prediction discrepancy explicitly so LLM writes CORRECTION ──
        if self._prev_expected:
            last_obs = ''
            for line in reversed(self.scratchpad.split('\n')):
                if line.startswith('Observation'):
                    last_obs = line.split(':', 1)[-1].strip()[:150]
                    break
            if last_obs:
                parts.append(
                    f"PREDICTION CHECK:\n"
                    f"You predicted: {self._prev_expected[:150]}\n"
                    f"Actual result: {last_obs}\n"
                    f"If these differ significantly, write a CORRECTION rule."
                )
        parts.append(STAR_STEP_INSTRUCTION)
        return '\n\n'.join(parts)
